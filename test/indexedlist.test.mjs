...
import IndexedList from '../js/indexedlist.mjs';
...
describe('IndexedList', () => {
    ...
    describe('count', () => {
        test('returns count for existing field/value', () => {
            assert.strictEqual(list.count('id', '1'), 1);
            assert.strictEqual(list.count('type', 'message'), 4);
            assert.strictEqual(list.count('conversation', 'A'), 2);
        });

        test('returns 0 for missing value', () => {
            assert.strictEqual(list.count('id', '99'), 0);
            assert.strictEqual(list.count('type', 'leave'), 0);
        });

        test('returns 0 for non-indexed field', () => {
            assert.strictEqual(list.count('unknown', 'x'), 0);
        });

        test('items with undefined field are not counted', () => {
            assert.strictEqual(list.count('conversation', undefined), 0);
        });
    });
